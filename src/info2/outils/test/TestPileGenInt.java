...
public class TestPileGenInt {
     ...
     */
    private static void testCreationPileVide(PileGenerique<Integer> p, String nomPile){
        if (p.estVide()) {
     ...
     * @param differente    pile avec une capacité différente des précédentes
     */
    private static void testCapacite(PileGenerique<Integer> p, PileGenerique<Integer> identique, PileGenerique<Integer> differente) {
     ...
     */
    private static void testEgalite(PileGenerique<Integer> p, PileGenerique<Integer> identique, PileGenerique<Integer> differente) {
    ...
    private static void testSansException() {
         ...
         */
        try {
            PileGenerique<Integer> p = new PileGenerique<>(5);                    // pile de référence
            PileGenerique<Integer> inverse = new PileGenerique<>(5);              // recevra l'inverse de p
            PileGenerique<Integer> grandePile = new PileGenerique<>(10);          // pour le test sur la capacité
    ...
    private static void testDesExceptions() {
        ...
        // l'exception ErreurCapaciteInvalide sera levée  (constructeur)
        try {
            new PileGenerique<>(-5);
        ...
        try {
            PileGenerique<Integer> p = new PileGenerique<>(5);
            for (int i = 1; i <= 6 ; i++) {
                p.empiler(i);
            }
