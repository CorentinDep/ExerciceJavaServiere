...
public class TestPileGenTel {
     ...
     */
    private static void testCreationPileVide(PileGenerique<Telephone> p, String nomPile){
        if (p.estVide()) {
     ...
     */
    private static void testCapacite(PileGenerique<Telephone> p, PileGenerique<Telephone> identique, PileGenerique<Telephone> differente) {

        if (PileGenerique.memeCapacite(p, identique)
                && !PileGenerique.memeCapacite(p, differente)
                && !PileGenerique.memeCapacite(identique, differente)) {
     ...
     */
    private static void testEgalite(PileGenerique<Telephone> p, PileGenerique<Telephone> identique, PileGenerique<Telephone> differente) {
    ...
    private static void testSansException() {
        ...
        try {
            ...
            for (int n = 1; n <= 5; n++) {
                p.empiler(new Telephone("0" + n + "58423645"));
            }
            System.out.println("La pile p contient-elle les entiers de 1 à 5 ?\n"
                    + p + "       => "
                    + (p.toString().equals(
                    "[ sommet = 0558423645 | 0458423645 | 0358423645 | 0258423645 | 0158423645 ]")
                    ? "OK " : "NOK") + "\n");
            ...
            System.out.println("Contenu de la pile inverse " + inverse + "    =>  "
                    ...
                    ("[ sommet = 0158423645 | 0258423645 | 0358423645 | 0458423645 | 0558423645 ]")
                    ? "OK " : "NOK") + "\n"
            ...
            p.empiler(new Telephone("060201050"));
            p.empiler(new Telephone("068425456"));
            System.out.println("Test " + (! inverse.equals(p) ? "ok " : "NOK")
                    + " pour equals");
            p.empiler(new Telephone("068454456"));
            System.out.println("Test " + (inverse.equals(p) ? "ok " : "NOK")
                    + " pour equals");
    ...
    private static void testDesExceptions() {
        ...
        try {
            PileGenerique<Telephone> p = new PileGenerique<>(5);
            for (int i = 1; i <= 6 ; i++) {
                p.empiler(new Telephone("0" + i + "65251285"));
            }
        ...
        // l'exception ErreurPileVide sera levée (méthode dépiler)
        try {
            PileGenerique<Telephone> p = new PileGenerique<>(5);
            p.depiler();
     ...
     */
    public static void main(String[] args) throws ErreurPile.ErreurCapaciteInvalide {
